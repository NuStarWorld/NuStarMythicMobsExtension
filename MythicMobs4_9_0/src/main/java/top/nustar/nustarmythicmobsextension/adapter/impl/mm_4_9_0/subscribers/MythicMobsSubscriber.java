...
public class MythicMobsSubscriber implements Listener {
    ...
    public void onTargetLoad(MythicTargeterLoadEvent event) {
        ...
        event.register((SkillTargeter) targetSelectorHelperServiceMap.get(targetSelectorType).findSelector(event.getConfig()));
    }
    ...
    public void onMythicMechanicLoad(MythicMechanicLoadEvent event) {
        ...
        event.register((SkillMechanic) mechanicHelperServiceMap.get(mechanicType).findMechanic(event.getConfig(), configService.getMainConfiguration()));
    }
        ...
        this.mechanicHelperServiceMap = mechanicHelperServices.stream()
                .filter(mechanicHelperService ->
                        mechanicHelperService.getClass().isAnnotationPresent(SupportMechanicType.class)
                                && mechanicHelperService.findMythicMobsVersionFromService(mechanicHelperService).equals(MythicMobsVersion.MM4_9_0)
                )
                .collect(Collectors.toMap(mechanicHelperService ->
                        mechanicHelperService.findMechanicTypeFromService(mechanicHelperService), Function.identity())
                );
